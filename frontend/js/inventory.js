...
async function addMedicine(medicineData) {
  try {
    ...
    // Validate data
    if (!medicineData.name || !medicineData.batch || !medicineData.expiryDate || 
        medicineData.quantity === undefined || medicineData.minThreshold === undefined) {
    ...
    if (existingMedicine) {
      ...
      const newQuantity = existingMedicine.quantity + parseInt(medicineData.quantity);
      const status = calculateMedicineStatus(medicineData.expiryDate, newQuantity, medicineData.minThreshold);
      
      // Update the existing medicine
      await db.collection('medicines').doc(existingMedicine.id).update({
        expiryDate: firebase.firestore.Timestamp.fromDate(new Date(medicineData.expiryDate)),
      ...
      await db.collection('transactions').add({
        ...
        reason: 'Added via duplicate prevention - merged with existing medicine',
        userId: user.uid,
    ...
    const status = calculateMedicineStatus(medicineData.expiryDate, medicineData.quantity, medicineData.minThreshold);
...
async function updateStock(medicineId, transactionType, quantity, reason = '') {
  try {
    ...
    // Recalculate status
    const status = calculateMedicineStatus(medicine.expiryDate.toDate(), newQuantity, medicine.minThreshold);
    ...
    await db.collection('transactions').add({
      ...
      quantity: transactionType === 'remove' ? -parseInt(quantity) : parseInt(quantity),
    ...
    // Update alerts
    await checkAndCreateAlerts(medicineId, { ...medicine, quantity: newQuantity, status });
...
async function checkAndCreateAlerts(medicineId, medicine) {
  try {
    const expiryDate = medicine.expiryDate?.toDate ? medicine.expiryDate.toDate() : new Date(medicine.expiryDate);
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    ...
    // Check for expiring soon
      ...
      await createOrUpdateAlert(medicineId, medicine.name, 'expiry_soon', expiry);
    }
    // Check for low stock
...
async function cleanupAlerts(medicineId, medicine) {
  try {
    ...
    alertsSnapshot.forEach(doc => {
      ...
      else if (alert.type === 'expiry_soon' && daysUntilExpiry > EXPIRY_WARNING_DAYS) {
        deletePromises.push(doc.ref.delete());
      }
      // Remove low_stock alert if stock is above threshold
      else if (alert.type === 'low_stock' && medicine.quantity > medicine.minThreshold) {
        deletePromises.push(doc.ref.delete());
      }
    });
...
async function getLowStockMedicines() {
  try {
    ...
    return allMedicines.filter(med => 
      med.status === 'low_stock' || med.quantity <= med.minThreshold
    );
  } catch (error) {
